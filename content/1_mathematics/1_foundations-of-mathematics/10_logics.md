...
命題論理だけでは表現しきれない数学的主張を記述するために、命題論理の体系に「個々の対象（モノ）」と「対象が満たす性質や関係」を記述するための語彙（量化記号など）を導入し、より表現豊かにした論理体系が一階述語論理である。
...
- **ステップ2：項の定義**
  変数記号や定数記号、それらに関数記号を適用して得られるものを「項」と定義する。項は「何らかの対象を指し示す表現」であり、論理式（真偽をもつ主張）とは異なる。
...
形式証明を構成する際に使える規則と公理を体系的にまとめる。ここでは「どのような規則が使えるか」を整理することに主眼を置く。これらの規則を用いた実際の証明の書き方は、次節「証明の形式」で扱う。
...
量化記号に関するドモルガンの法則は、「全称」と「存在」を否定によって相互に変換する。直感的には、「すべての $v$ について $\phi$ でない」ことと「$\phi$ を満たす $v$ が存在しない」ことは同じ意味であり、「ある $v$ について $\phi$ でない」ことと「$\phi$ がすべての $v$ について成り立つわけではない」ことも同値である。
...
> **注意：$\forall$と$\exists$の結びつく論理記号**
>
> 全称量化と存在量化は、結びつく論理記号が異なる。
> - 「すべての$x$について$P(x)$かつ$Q(x)$」は $\forall x,(P(x) \land Q(x))$ だが、「すべての$x$について$P(x)$、かつすべての$x$について$Q(x)$」は $\forall x,P(x) \land \forall x,Q(x)$（これらは同値）。
> - 「ある$x$が存在して$P(x)$かつ$Q(x)$」は $\exists x,(P(x) \land Q(x))$ であり、「ある$x$が存在して$P(x)$、かつある$x$が存在して$Q(x)$」すなわち $\exists x,P(x) \land \exists x,Q(x)$ とは一般に異なる（前者は後者を含意するが、逆は成立しない）。
...
- **主張の論理構造を把握する:** 量化記号がすべて先頭に集まることで、「何を全称的に主張し、何を存在的に主張しているか」の骨格が一目で見える。「任意の $x$ に対してある $y$ が存在して……さらに任意の $z$ について……」のように量化記号が入り組んだ主張も、冠頭標準形にすれば $\forall x,\exists y,\forall z,\cdots$ と整理される。
...
> **代数学における応用:**
>
> これらの論理的同値性は、群論において「群の単位元がただ一つであること」や「ある元の逆元が一意であること」を厳密に証明する際などに必要な概念である。