...
[ノート：実数（ℝ）](40_knowledge-base/10_mathematics/20_set-theory/80-reals/)で得た $\mathbb{R}$ の上で、数列の収束を厳密に定義する。$\varepsilon$-$N$ 論法による収束の定義を出発点に、極限の基本性質を整理し、完備性（上限性質）と同値な諸命題を導く。
...
>
>厳密な意味は、[ノート：一階述語論理の記法と推論規則](40_knowledge-base/10_mathematics/10_foundations-of-mathematics/10_logics/)参照
...
[ノート：実数](40_knowledge-base/10_mathematics/20_set-theory/80-reals/)で定義した完備性（上限性質）が、数列の収束に関する以下の命題と同値であることを示す。
