...
[ノート：数体系の構成（ℤ・ℚ）](40_knowledge-base/10_mathematics/20_set-theory/60-numbers/)や[ノート：実数（ℝ）](40_knowledge-base/10_mathematics/20_set-theory/80-reals/)で構成したℤ・ℚ・ℝは、この抽象的な定義の具体例である。ℤからℚへの拡張は、乗法逆元を付け加える操作であったが、これは環から体への拡張として整理できる。
...
$\mathbb{Z}$ は可換環である。(R1)〜(R4) は、すべて[ノート：数体系の構成（ℤ・ℚ）](40_knowledge-base/10_mathematics/20_set-theory/60-numbers/)で確認済みである。乗法の単位元は $1$。
